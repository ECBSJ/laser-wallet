  ...
  request: async function (method, params) {
    ...
    return new Promise(function (resolve, reject) {
      function handleMessageFromLaserProviderType(event) {
        ...
        console.log("[LaserProvider]", response);
        window.removeEventListener("message", handleMessageFromLaserProviderType);
        if ("error" in response) {
          reject(response);
          console.log("[LaserProvider] Confirmation Error: ", response.error);
        } else {
          resolve(response);
          console.log("[LaserProvider] Confirmation Result: ", response.result);
        }
      }
      window.addEventListener("message", handleMessageFromLaserProviderType);
    });
  },
};
...
console.log(`[LaserProvider] ${window.LaserProvider.isLaser ? "Injected!" : "Injection failed."}`);
